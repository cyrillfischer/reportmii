// supabase/functions/analysis-started-mail/index.ts
...
serve(async (req) => {
  ...
  const GROUP_ID = Deno.env.get("MAILERLITE_ANALYSIS_STARTED_GROUP_ID");

  if (!MAILERLITE_API_KEY) return jsonResponse(500, { error: "Missing secret: MAILERLITE_API_KEY" });
  if (!GROUP_ID) return jsonResponse(500, { error: "Missing secret: MAILERLITE_ANALYSIS_STARTED_GROUP_ID" });
  ...
  try {
    ...
    // 2) Assign subscriber to group (triggers automation Mail 1)
    ...
    return jsonResponse(200, {
      ...
      subscriber_id: subscriberId,
      group_id: GROUP_ID,
      note: "Analysis started mail will be sent by MailerLite automation (group trigger).",
    });
  } catch (err) {
    return jsonResponse(500, {
